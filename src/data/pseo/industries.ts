/**
 * Industry configurations for website speed audits
 */
...
export function getIndustryBySlug(slug: string): IndustryConfig | undefined {
  return INDUSTRIES.find(ind => ind.industry === slug);
}

export function getIndustriesForPlatform(platform: string): IndustryConfig[] {
  return INDUSTRIES.filter(ind => ind.commonStack.includes(platform));
}
