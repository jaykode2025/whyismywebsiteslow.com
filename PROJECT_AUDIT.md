...
- **The static `public/robots.txt`** points to a sitemap URL that both 308-redirects (wrong host) and 404s (wrong filename) — this quietly damages discoverability of everything else that's actually well-built (the pSEO tree).
...
8. **Removed `dompurify` from `package.json` dependencies** (unused — `isomorphic-dompurify` is the one actually imported and already bundles it).
